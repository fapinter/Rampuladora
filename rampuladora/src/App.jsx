...
const App = () => {
  ...
  function getLandings(inc, size) {
    ...
    if (checkIntermediates) {
      ...
      if (inc <= 5) {
        step = 1.5;
      } else if (inc > 5 && inc <= 6.25) {
        step = 1;
      } else if (inc > 6.25 && inc <= 8.33) {
        step = 0.8;
        setLimitateSegments(true);
      }

      if (!isNaN(step)) {
        for (
          let curr_height = step;
          curr_height <= height;
          curr_height += step
        ) {
    ...
    if (checkEvery50m) {
      ...
      for (let curr_step = 50; curr_step <= temp_size; curr_step += 50) {
        const landings_in_between = patamares.filter((item) => {
          if (item > last_step && item <= curr_step) {
            return item;
          }
        });
  ...
  function calculate() {
    ...
    if (type_calculator === "comprimento") {
      inclination_percentage =
        inclinationUnit === "degrees"
          ? Math.tan(inclination) * 100
          : inclination;
      inclination_degrees =
        inclinationUnit === "degrees"
          ? inclination
          : parseFloat(
              (
                Math.atan(inclination_percentage / 100) *
                (180 / Math.PI)
              ).toFixed(2),
            );
      ...
      inclination_degrees = parseFloat(
        (Math.atan(inclination_percentage / 100) * (180 / Math.PI)).toFixed(2),
      );
      size = length;
    }
    switch (exception) {
    ...
    const [final_size, num_segments, num_landings] = getLandings(
      inclination_percentage,
      size,
    );
  ...
  return (
    <div className="container">
      <div
        style={{ height: "7.5%", marginBottom: "2.5%", textAlign: "center" }}
      >
        <h1>A Rampuladora</h1>
      </div>
          ...
          <div className="input-wrapper">
            <label>
              ...
              <span style={{ fontSize: "14px" }}>
                (interferem na inclinação máxima)
              </span>
            </label>
            <select
              className="input-field"
          ...
          {exception === "default" ? (
            ...
            <div className="checkbox-wrapper">
              <span style={{ "font-size": "20px" }}>
                Patamares não aplicáveis a este cenário
              </span>
            </div>
          )}
